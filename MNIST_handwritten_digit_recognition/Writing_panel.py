...
class Penal(object):
    ...
    def addText(self):
        ...
        self.screen.blit(self.font.render('Keyboard input : c=clean up ; s=save  ' , True, (255,255,255) ),  (10, 35))
        self.screen.blit(self.font.render('e=export (as json) ; o : operate recongition  ; ESC = exit ' , True, (255,255,255) ),  (10, 60))
    ...
    def print_out_used_NN(self):
        ...
        self.screen.blit(self.font.render( "Used neural network : "+self.which_Neural_Network    , True, (255,255,255) ), (60, 700))
        pygame.display.update()
                ...
                while True:
                    ...
                    if pygame.time.get_ticks() -  self.start_ticks > 500 :
                        ...
                        if keypressed[pygame.K_e]:
                            print("==================================================")
                            print("export as test.json --")
                            self.save_into_json()
                            self.start_ticks = pygame.time.get_ticks()
                            print("export as test.json -- completed!")
                            print("==================================================")
                        if keypressed[pygame.K_o]:
                            ...
                            if  self.which_Neural_Network == 'FullConnectLayer' :
                                if self.import_FCL == True :
                                    importlib.reload(FullConnectLayer_predict_only)
                                else :
                                    self.import_FCL = True
                                    import FullConnectLayer_predict_only
                                result = str(FullConnectLayer_predict_only.Get_result()[0] )
                            if  self.which_Neural_Network == 'ConvolutionLayer' :
                                if self.import_CNN == True :
                                    importlib.reload(ConvolutionLayer_predict_only)
                                else :
                                    self.import_CNN = True
                                    import ConvolutionLayer_predict_only
                                result2 = str(ConvolutionLayer_predict_only.Get_result()[0] )
                            if  self.which_Neural_Network == 'Both' :

                                if self.import_CNN == True :
                                    importlib.reload(ConvolutionLayer_predict_only)
                                if self.import_CNN == False :
                                    self.import_CNN = True
                                    import ConvolutionLayer_predict_only
                                result2 = str(ConvolutionLayer_predict_only.Get_result()[0] )
